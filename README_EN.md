...
| [`presentacion/Claude_Code_Presentacion_EN.pptx`](./presentacion/) | The deck in English (16:9, **40 slides**, level with the Spanish one). Rebuild with `python presentacion/build_pptx.py --lang en && python presentacion/merge_manual_slides.py --lang en`. |
| [`presentacion/translations_en.py`](./presentacion/translations_en.py) | The ES→EN line table `--lang en` uses. There is **one** layout for both languages: the deck is always drawn in Spanish and translated as it is emitted, so adding a slide adds it to both decks and the only thing that can be missing is its translation — which the build lists at the end rather than swallowing. |
