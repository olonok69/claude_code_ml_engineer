...
Claude Code session must know *which machine it is on* before it acts. Otherwise a
contributor machine will helpfully rebuild and republish the shared graph — the one
thing its role forbids — and be entirely pleased with itself for doing so.
...
`CLAUDE.md` points at it, so every session reads its own role first.
